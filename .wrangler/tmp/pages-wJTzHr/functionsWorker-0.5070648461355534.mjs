...
var onRequestDelete3 = /* @__PURE__ */ __name(async (ctx) => {
  const g = await guardAdmin(ctx);
  if (!g.ok) return g.res;
...
// api/admin/users_reset_password.ts
...
__name(randomSaltHex2, "randomSaltHex");
...
var onRequestGet8 = /* @__PURE__ */ __name(async (ctx) => {
  const u = await getUser(ctx);
  if (!u) return new Response("Unauthorized", { status: 401 });
  const type = ctx.params.type;
...
var onRequestGet9 = /* @__PURE__ */ __name(async (ctx) => {
  const u = await getUser(ctx);
  if (!u) return new Response("Unauthorized", { status: 401 });
...
var onRequestGet11 = /* @__PURE__ */ __name(async (ctx) => {
  const u = await getUser(ctx);
  if (!u) return new Response("Unauthorized", { status: 401 });
  const month = new URL(ctx.request.url).searchParams.get("month");
  if (!month) return new Response("Missing month", { status: 400 });
...
// api/liste.ts
var onRequestGet12 = /* @__PURE__ */ __name(async (ctx) => {
  const url = new URL(ctx.request.url);
...
// api/rapportini.ts
var onRequestGet13 = /* @__PURE__ */ __name(async (ctx) => {
  const u = await getUser(ctx);
  if (!u) return new Response("Unauthorized", { status: 401 });
  const month = new URL(ctx.request.url).searchParams.get("month");
  if (!month) return new Response("Missing month", { status: 400 });
...
// ../.wrangler/tmp/pages-wJTzHr/functionsRoutes-0.17565514669832205.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-uWcVrE/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-uWcVrE/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5070648461355534.mjs.map
